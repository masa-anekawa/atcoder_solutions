...
int N,M,K,A,B,C,X[NMAX];
int dp[NMAX] = {};
ll dist[NMAX][2];
int G[NMAX][2][2]; // G[node][from_var][to_var]
p V[NMAX][2] = {};
...
void solve(){
  ...
  for (int i = 0; i < N; ++i)
  {
      dist[i+1][0] = min(dist[i][0] + G[i][0][0],
                         dist[i][0] + G[i][1][0]);
      dist[i+1][1] = min(dist[i][1] + G[i][0][1],
                         dist[i][1] + G[i][1][1]);
  }
}
void debug(){
...
int main(int argc, char const *argv[])
{
  ...
  {
      scanf("%d", &X[i]);
      V[i][0] = p(X[i], V[i-1][0].second);
      if(X[i] == V[i-1][1].first)
  }
  solve();
  #ifdef DEBUG
  debug();

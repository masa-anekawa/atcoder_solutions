...
void solve(){
  ...
  for (int i = 0; i < N; ++i)
  {
    S[i+1] = S[i] + abs(A[i] - (i > 0 ? A[i-1] : 0LL));
  }
  for (int i = 0; i < N ; ++i)
  {
    S_rev[N-i] = S_rev[N-i + 1] + abs(A[N-i - 1] - (i > 0 ? A[N-i] : 0LL));
  }
  ...
  for (int i = 1; i <= N ; ++i)
  {
    if (i == 1)
    {
      ans[i] = S[i-1] + abs(A[i] - 0LL) + S_rev[i+1];
    }else if(i == N){
      ans[i] = S[i-1] + abs(0LL - A[i-2]) + S_rev[i+1];
    }else{
      ans[i] = S[i-1] + abs(A[i] - A[i-2]) + S_rev[i+1];
    }
  }
}
void debug(){

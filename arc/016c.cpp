...
ll N,M,K,A[MMAX],B[MMAX],I[MMAX][NMAX]={}, P[MMAX][NMAX]={};
double dp[1 << NMAX];
std::map<int, double> mp[MMAX];
string S;
vec v;
...
double solve(int s){
  ...
  for(int j = 0; j < M; ++j){
      double p_sum = accumulate(mp[j].begin(), mp[j].end(), 0.0,
          [&v](double a, pair<int, double> b){return v.count(b.first) ? a + b.second : a;});
      DUMP(p_sum);
      if(!p_sum) continue;
      // cand = min(cand,
      double cand = B[j] / p_sum;
      for(auto &&i : v){
          if(mp[j].count(i) == 0) continue;
          cand += solve(s | 1 << i) * mp[j][i] / p_sum;
      }
      ret = min(ret, cand);
  }
  // cout << "s: " << bitset<5>(s) << " costs " << ret << endl;
  return dp[s] = ret;
}
void debug(){
...
int main(int argc, char const *argv[])
{
  ...
  for (int i = 0; i < M; ++i)
  {
      ...
      for (int j = 0; j < A[i]; ++j)
      {
          scanf("%lld%lld", &I[i][j], &P[i][j]);
          mp[i].insert(make_pair(I[i][j] - 1, P[i][j] / 100.0));
      }
  }
  ans = solve(0);
  #ifdef DEBUG

...
void solve(){
  ...
  for (int k = 1; k <= N; ++k)
  {
      for (int i = 1; i <= N; ++i)
      {
          for (int j = 1; j <= N; ++j)
          {
              if(dp[i][j].cost > dp[i][k].cost + dp[k][j].cost){
                  set<int> st(dp[i][k].route);
                  st.insert(dp[k][j].route.begin(), dp[k][j].route.end());
                  dp[i][j] = path{
                      dp[i][k].cost + dp[k][j].cost,
                      st
                  };
              }
          }
  ...
  for (int i = 1; i <= N; ++i)
  {
      for (int j = 1; j <= N; ++j)
      {
          all.insert(dp[i][j].route.begin(), dp[i][j].route.end());
      }
  }
  ans = M - all.size();
}
void debug(){
  ...
  for (int i = 1; i <= N; ++i)
  {
      for (int j = 1; j <= N; ++j)
      {
          ...
          auto route = dp[i][j].route;
          debug("dp[%d][%d] cost: %d\n\troute: ", i, j, cost);
          for(auto&& i : route) {
              debug("(%d, %d) ", E[i].first, E[i].second);
          }
          cout <<endl;
      }
  }

...
void solve(){
  ...
  {
      // plus
      ll s = S[i] + dp[i-1][1].first - dp[i-1][1].second;
      if (s <= 0)
      {
          dp[i][0] = llp(dp[i-1][1].first + 1 - s, dp[i-1][1].second);
      }else{
          dp[i][0] = dp[i-1][1];
      }
      // minus
      s = S[i] + dp[i-1][0].first - dp[i-1][0].second;
      if (s >= 0)
      {
          dp[i][1] = llp(dp[i-1][0].first, dp[i-1][0].second + s + 1);
      }else{
          dp[i][1] = dp[i-1][0];
      }
  }
...
void debug(){
  ...
  for (int i = 0; i < 2; ++i)
  {
      for (int j = 1; j <= N; ++j)
      {
          printf("(%lld, %lld) ", dp[j][i].first, dp[j][i].second);
      }
      cout <<endl;
  }
}
void answer(){
  // output answer
  printf("%lld\n", min(dp[N][0].first + dp[N][0].second,

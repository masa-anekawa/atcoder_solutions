...
ll N,M,K,A,B,C,D,E;
int dp[NMAX][NMAX / 2 + 1] = {};
string Str;
ll ans = 0;

inline int battle(int index, char hand){
  if (Str[index] == hand)
  {
      return 0;
  }else if(Str[index] == 'p' && hand == 'g'){
      return -1;
  }else{
      return 1;
  }
}
ll solve(int index, int restPs){
  debug("solve(%d, %d)\n", index, restPs);

  if(dp[index][restPs] != -1) return dp[index][restPs];
  if(restPs < 0) return -INF;
  if(index >= N) return 0;

  dp[index][restPs] = max(
      solve(index+1, min(restPs+1, NMAX / 2)) + battle(index, 'g'),
      solve(index+1, restPs-1) + battle(index, 'p'));
  debug("dp[%d][%d] = %d\n", index, restPs, dp[index][restPs]);
  return dp[index][restPs];
}
void debug(){
  // デバッグ用出力
}
